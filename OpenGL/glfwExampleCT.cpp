...
int main(void)
{
    ...
    Shader.addShader( "fragmentShader_C-T.glsl", sivelab::GLSLObject::FRAGMENT_SHADER );
    ...
    GLuint projMatrixID, viewMatrixID, modelMatrixID, normalMatrixID, lightPosID, diffuseComponentID, specularComponentID, shininessID, cameraPosID, shadingModeID, useFlatColorID, flatColorID, roughnessID, metallicID, albedoID;
    ...
    roughnessID = Shader.createUniform( "roughness" );
    metallicID = Shader.createUniform( "metallic" );
    albedoID = Shader.createUniform( "albedo" );
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3fv(albedoID, 1, glm::value_ptr(albedo));
        ...
        glUniform3fv(albedoID, 1, glm::value_ptr(glm::vec3(0.8f, 0.8f, 0.8f)));
        ...
        for(int i = 0; i<spherePositions.size(); i++){
            ...
            glUniform3fv(albedoID, 1, glm::value_ptr(color));
